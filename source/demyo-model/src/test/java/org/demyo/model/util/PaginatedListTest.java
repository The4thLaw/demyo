...
class PaginatedListTest {
	...
	void testMaxPages() {
		...
		list = new PaginatedList<>(EMPTY_LIST, 1, 10, 9);
		assertThat(list.getMaxPages()).isEqualTo(1);

		list = new PaginatedList<>(EMPTY_LIST, 1, 10, 10);
		assertThat(list.getMaxPages()).isEqualTo(1);

		list = new PaginatedList<>(EMPTY_LIST, 1, 10, 11);
	...
	void testPreviousNext() {
		...
		list = new PaginatedList<>(EMPTY_LIST, 1, 10, 95);
		assertThat(list.hasPreviousPage()).isFalse();
		assertThat(list.hasNextPage()).isTrue();

		list = new PaginatedList<>(EMPTY_LIST, 5, 10, 95);
		assertThat(list.hasPreviousPage()).isTrue();
		assertThat(list.hasNextPage()).isTrue();

		list = new PaginatedList<>(EMPTY_LIST, 10, 10, 95);
		assertThat(list.hasPreviousPage()).isTrue();
